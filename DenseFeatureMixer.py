...
class DenseFeatureMixer(BaseEstimator, TransformerMixin):
    ...
    def fit(self, x_dataset,y_dataset, num_columns, cat_columns, dim):
        ...
        uniques_idx = {col: x_dataset[col].drop_duplicates().index for col in cat_columns}
        ...
        self.frames = {cat_columns[k]: [str(cat_columns[k])+'_embedding_'+str(i) for i in range(2)] for k in range(len(cat_columns))}

        frames_col_names = {cat_columns[k]: [str(cat_columns[k])+'_embedding_'+str(i) for i in range(2)] for k in range(len(cat_columns))}

        for i in weights:
            frames_content = [pd.DataFrame(mapped_weights.get(i),columns=frames_col_names.get(i),index=uniques_idx.get(i)) for i in mapped_weights]

        mapped_frames = dict(zip(cat_columns,frames_content))

        self.datasets  = [mapped_frames.get(i).join(x_dataset[i], how='left') for i in mapped_frames]
    ...
    def transform(self, x_dataset):
        ...
        final_datasets = [x_dataset.merge(self.datasets[i],how='inner',on=self.cat_columns[i],copy=False) for i in range(len(self.datasets))]

        for i in range(len(final_datasets)):
            final_datasets[i].drop(x_dataset.columns,axis=1,inplace=True)
        
        for i in range(len(final_datasets)):
            x_dataset = x_dataset.merge(final_datasets[i],left_index=True,right_index=True,how='inner')
